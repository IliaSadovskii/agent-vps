...
OPS_DIR="/home/$DEV_USER/ops"   # «кабинет» управляющей сессии (настройка сервера)
...
install_parallel_sessions() {
  ...
  # Из vps-main (ops) → в ops. Изнутри проекта → в папке проекта.
...
main() {
  ...
  # --- Папка управляющей сессии (настройка сервера) ---
  if [[ -d "$OPS_DIR" ]]; then
    skip "Папка управления $OPS_DIR"
  else
    say "Создаю $OPS_DIR (кабинет управляющей сессии)…"
    install -d "$OPS_DIR" -o "$DEV_USER" -g "$DEV_USER" -m 755
    # CLAUDE.md с ролью — чтобы управляющий агент понимал, кто он
    cat > "$OPS_DIR/CLAUDE.md" <<'OPSMD_EOF'
# Роль этой сессии

Ты — управляющий агент этого VPS. Твоя задача — настройка и обслуживание
сервера: установка софта, веб-сервер, домены, SSL, systemd-сервисы, firewall,
подготовка окружения под проекты.

- У тебя есть sudo без пароля — можешь настраивать что угодно на сервере.
- Код конкретных проектов живёт в /projects — туда для разработки создаются
  отдельные параллельные сессии (командой claude-new). Сам код проектов ты
  обычно не пишешь здесь; здесь — инфраструктура и настройка.
- Держи в этой папке заметки о том, что уже настроено на сервере.
OPSMD_EOF
    chown "$DEV_USER:$DEV_USER" "$OPS_DIR/CLAUDE.md"
    ok "Папка управления готова: $OPS_DIR"
  fi
...
EOF
  ...
  echo "==============================================================="
  echo " Основная сессия в приложении: ${C_INFO}vps-main${C_RST} (живёт в ~/ops, управляет сервером)"
