#!/usr/bin/env bash
# ============================================================================
#  dev-env.sh — ПЛАТФОРМА РАЗРАБОТКИ на этом VPS
# ============================================================================
#
#  Отдельно от vps-bootstrap.sh намеренно: bootstrap — это скрипт «получить
...
#
#  Запуск:  bash dev-env.sh          (под обычным пользователем, sudo внутри)
...
step_firewall() {
  ...
  sudo ufw status 2>/dev/null | grep -q "Status: active" \
    || die "UFW не активен — его включает vps-bootstrap.sh."
  ...
  local marker="# --- DOCKER-USER (dev-env.sh) ---"
  local rules_file
  for rules_file in /etc/ufw/after.rules /etc/ufw/after6.rules; do
    if sudo grep -qF "$marker" "$rules_file"; then
...
main() {
  ...
  sudo -n true 2>/dev/null || die "Нужен passwordless sudo (его настраивает vps-bootstrap.sh)."
